$(document).ready(function(){
        ...
        function(data, status){
        	if(status =='success' & data==1){
        	    M.toast( {html: 'Votaciones guardadas', displayLength:1000, completeCallback: function(){
					window.location="http://192.168.0.13/votacionesmaranata/adolescentes";
				}});
	...
	$("input[type=checkbox]").change(function(){
		...
		var cantidadMaxima=4; //limite de checkbox seleccionables
		...
		{
            M.toast({html: 'Solamente puedes seleccionar 4',displayLength:1200})
